    ...
    <div class="rounded-lg shadow-all-side w-[80%] my-5 p-8 mx-auto">
        <form class="mx-10" action="{{ route('management-users-update', ['id' => $User->id]) }}" method="POST">
            ...
            <div class="mb-2">
                ...
                <input type="text" id="nama" name="name" value="{{ $User->name }}"
            ...
            <div class="mb-2 ">
                ...
                <select name="role_id" id="role"
                    class="border-none bg-gray-300 border border-gray-300 text-gray-900 text-sm rounded-full outline-none ps-5 block w-full p-1.5">
                    <option value="{{ $User->role_id }}" class="rounded-none py-2 px-2">{{ $User->RelasiRoles->name }}
                    </option>
