...
<div class="rounded-lg shadow-all-side w-[80%] my-5 p-8 mx-auto">
    <form class="mx-10" action="{{ route('management-users-store') }}" method="POST">
      @csrf
        ...
        <div class="mb-2">
          ...
          <input type="text" id="nama" name="name" class="bg-gray-300 border border-gray-300 text-gray-900 text-sm rounded-full outline-none ps-5 block w-full p-1.5"/>
        </div>
        <div class="mb-2">
          <label for="password" class="block mb-2 text-sm text-gray-900 font-semibold">Password</label>
          <input type="password" id="password" name="password" class="bg-gray-300 border border-gray-300 text-gray-900 text-sm rounded-full outline-none ps-5 block w-full p-1.5"/>
        </div>
        <div class="mb-2">
          <label for="email" class="block mb-2 text-sm text-gray-900 font-semibold">Email</label>
          <input type="email" id="email" name="email" class="bg-gray-300 border border-gray-300 text-gray-900 text-sm rounded-full outline-none ps-5 block w-full p-1.5"/>
        </div>
        <div class="mb-2 ">
          ...
          <select name="role_id" id="role" class="border-none bg-gray-300 border border-gray-300 text-gray-900 text-sm rounded-full outline-none ps-5 block w-full p-1.5">
            ...
            <option value="{{ $item->role_id }}" class="rounded-none py-2 px-2">{{ $item->RelasiRoles->name }}</option>
            @endforeach
          </select>
        </div>
