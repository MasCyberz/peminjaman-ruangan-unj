        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-putihan rounded-lg shadow-all-side">
                <div class="flex items-center p-4 border-b border-gray-700 rounded-t">
                    <a href="{{ route('pengajuan_koordinator') }}"><button type="button"
                ...
                <div class="p-4 md:p-5 space-y-4">
                    <div class="flex">
                        ...
                        <div class="flex flex-col z-10">
                            <span class="block mt-3 mx-3 text-base font-semibold">{{ $surat->nomor_surat }}</span>
                            <span class="block mt-3 mx-3 text-base font-semibold">{{ $surat->asal_surat }}</span>
                        ...
                        @endforeach --}}
                                @foreach ($formattedData as $item)
                                    <span class="block -ms-5     text-base font-semibold">{{ \Carbon\Carbon::parse($item['tanggal_peminjaman'])->format('d F Y') }}</span>
                                    @foreach ($item['ruang_peminjaman'] as $items)
                                        <ul>
                                            <li class="list-disc">
                                                {{ $items['nomor_ruang'] }}
                                            </li>
